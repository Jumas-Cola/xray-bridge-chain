...
* `client/`: Шаблон клиентского конфига с routing (российские домены и IP идут напрямую, остальной трафик — через прокси).
* `generate-keys.sh`: Скрипт для автоматизации генерации необходимых ключей и сертификатов.


## Запуск

1. Запустить скрипт generate-keys.sh и сохранить сгенерированные значения и клиентский конфиг.
...
4. Использовать сгенерированный клиентский конфиг (заменив `BRIDGE-SERVER-IP` на IP bridge-сервера).
